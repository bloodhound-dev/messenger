...
## 7) Production dashboard checks (web)

- Confirm KPI cards populate for 24h sent/received/peak users/latency/errors.
- Confirm both line charts render and update after sending messages.
- Confirm regional distribution bars and world map markers are visible.
- Confirm security posture panel shows OTP policy + key rotation age status.
- Confirm security audit table records events (OTP requests, login, message sends).
