...
- This repo includes `vercel.json` with baseline security headers (CSP, X-Frame-Options, etc.).
- Root `/index.html` redirects to `/web/index.html` to avoid Vercel root 404 issues.
- See `DEPLOYMENT.md` troubleshooting if you get `404: NOT_FOUND`.


## Production web dashboard enhancements

The web dashboard now includes:
- 24h KPI cards (traffic, peak users, latency, error rates)
- Throughput/latency trend chart
- CPU and memory trend chart
- Regional user-distribution bars + global map markers
- Security posture panel (OTP policy, key age/rotation status)
